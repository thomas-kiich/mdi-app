 ...
 * Version 2 – ohne Blobs am Wortende
 *
 * Strategie gegen Blobs:
 * - Kein Komma-Trick (fuegeSprechpausenEin) – der erzeugt Artefakte
 * - Stattdessen: kurze Sätze mit natürlichen Pausen durch Leerzeilen
 * - Text endet sauber mit Punkt (kein Trailing-Whitespace)
...
// Sprechertext – sauber strukturiert, keine Komma-Tricks
// "kich" für korrekte Aussprache, kurze Sätze für natürlichen Fluss
const SPRECHERTEXT = `Willkommen bei kich.

Öffne in deinem Browser www.kich.de.

Lies dir den Text auf der Seite in Ruhe durch.

Dann folgen vier einfache Schritte.

Schritt eins. Lies die Datenschutzerklärung und setze das Häkchen bei der Einwilligung.

Schritt zwei. Klicke auf den Button: Jetzt kostenlos bei kich starten.

Schritt drei. Gib deine E-Mail-Adresse ein. Keine Kreditkarte, kein Abo.

Schritt vier. Bestätige deine Identität und wähle dein Konto aus.
...
async function generateAudio() {
  ...
  console.log("🎙️ Generiere Audio mit MA-Stimme (Voxtral) – Version 2...");
  ...
  const outputPath = path.join(__dirname, "../registrierung-anleitung-ma-stimme-v2.mp3");
