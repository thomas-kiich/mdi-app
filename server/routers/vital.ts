...
export const vitalRouter = router({
  ...
  saveEintrag: protectedProcedure
    .input(z.object({
      ...
      ruhepuls: z.number().int().min(20).max(300).nullable().optional(),
      hrv: z.number().int().min(0).max(500).nullable().optional(),
      apnoeAus: z.string().max(10).nullable().optional(),
      apnoeEin: z.string().max(10).nullable().optional(),
      bolt: z.number().int().min(0).max(600).nullable().optional(),
